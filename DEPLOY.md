...
# PROBE_INTERVAL_S / RCLONE_TIMEOUT_S / PROBE_FAIL_THRESHOLD are optional — compose supplies the defaults
# (300 / 240 / 2), so an EXISTING .env from before 2026-09-11 needs no edit. See DESIGN.md
# "Probe cadence and flap damping" before changing any of the three.
...
| Read-only rclone remote's token revoked/expired | Every `db_snapshot` probe errors → `dashboard-probes` `FAIL` (after `PROBE_FAIL_THRESHOLD` cycles) with `rclone exit …` in the note; `dest.probe_error` on the cards | Re-run the `rclone authorize` flow in §1b; `RCLONE_CONFIG=~/.config/rclone/dashboard-ro.conf rclone lsd gdrive-ro:` on the box |
| Drive rate-limits a big-tree probe (`rateLimitExceeded` / rclone timeout) | A cycle fails but `dashboard-probes` stays **OK** — by design, damped until `PROBE_FAIL_THRESHOLD` (2) consecutive cycles. Before this existed it flapped FAIL→OK 27×/4 days and buried a real failure | The errors are still recorded: `reason` says `probe-error damped (transient quota/timeout, cycle 1 of 2)`, `metrics.failed_transient` counts them, and the job page's probe table shows each `transient (Drive quota/timeout): …` row. If it never clears, the 2nd cycle trips FAIL and pages. Reduce traffic with `probe.interval_s` on the big tree before raising the threshold. |
